...
| `penumbra_graph` | The memory of relations: `find` (name → node ids) → `stats` → `neighborhood`. Identity is an evidence-carrying edge; collapse policies are `conservative` \| `working` \| `exploratory`. |
