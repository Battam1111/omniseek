...
| `penumbra_graph` | The memory of relations, ONE stable verb: `penumbra_graph(view, args={...})` (no view = the live view catalog; a wrong argument names the view's real parameters). Views: `find` (name → node ids) → `stats` → `neighborhood` → `between` (bounded connection paths between two anchors) → `voices` (collapse a doc set to distinct upstream voices: the independence counter; docs with zero evidence land in `unresolved`, never counted) → `since` (the accretion log: what accrued around a node after a date, tier + method shown, no collapsing) → `similar` (vector-nearest doc candidates for an anchor doc, by rank, no scores: proposals only, never collapsed by any policy; ratify with `penumbra_ruling`). Identity is an evidence-carrying edge; collapse policies are `conservative` \| `working` \| `exploratory`. |
| `penumbra_ruling` | Record / list / retract your identity rulings (`same` \| `not_same` on a node pair): the identity half of the judgment channel, applied by the graph's `working` policy. The pair is the key; re-creating a pair replaces the prior verdict. Penumbra stores your judgment, it never makes one. |
| `penumbra_statement` | Record / list / retract your typed RELATION statements: directed agent judgments (`src`, `dst`, free `type`, required `note`, optional provenance `doc`) that project in `neighborhood` / `between` / `since` under `working` and `exploratory`, never `conservative`. The directed triple is the key; identity types are refused (use `penumbra_ruling`); endpoints may name entities no source ever minted (label-keyed ids are findable). |
| `penumbra_sensor` | Standing queries with novelty detection: `action=create/list/delete/run`, optional `notify` for push on new results. Sensors run on their schedule automatically in the live service (`hourly` \| `daily` \| `weekly`); `action="run"` is the manual trigger. |
