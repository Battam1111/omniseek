...
<sub>[penumbra](../README.md)&nbsp;·&nbsp;[Configuration](configuration.md)&nbsp;·&nbsp;[Tools](tools.md)&nbsp;·&nbsp;[Patterns](patterns.md)&nbsp;·&nbsp;[Case study](case-study.md)</sub>

Short answers to the questions people ask first. The one-line version of what Penumbra is:
...
A search wrapper answers a query and forgets it; Penumbra answers a query and keeps it. The
...
- **Penumbra** is the fourth corner: your index, your judgments, your model's thinking. It
...
Penumbra does not compete with an open-web search API on breadth: those index billions of
pages, and Penumbra curates a couple hundred sources chosen to beat plain search via a mode
...
half-lit corners with Penumbra.
...
and an extraction mistake becomes a "fact" in the graph. Penumbra stores mechanical facts and
...
out of the box; Penumbra will not summarize anything (it never generates), and it reaches the
...
Never. Penumbra binds to loopback, requires a bearer token on every request, and makes no
...
session: Penumbra reaches only what your own account can already see, using your own credentials
...
No, by design. Penumbra refuses three things permanently: it never generates (there is no model
inside, so nothing hallucinated enters your data), it never judges (what counts as the same
thing, what relates to what, what matters, your agent decides and Penumbra records the decision
with its evidence), and it never phones home. In a stack where every other layer is a model that
can hallucinate, Penumbra is the one layer that structurally cannot, because it never generates.
