...
表層の下には:あるポッドキャストの47分目に、あなたが探している洞察がある。文字起こしされたことはない。それを再現させる肝心の設定は、スライドに3秒だけ映って消えた。その資料は公開されなかった。あなたが「新しい」と呼ぼうとしている手法は、何年も前に、あなたの読めない言語で解かれていた。
...
<div align="center">
  <picture>
    ...
    <img src="../../assets/demo-light.png" alt="実例:エージェントが、ある結果がなぜ再現しないのかを尋ねる。通常の検索(論文・README・ブログ)はどれも「動作する」と言う。Penumbraは表層の下に手を伸ばし、表層が取りこぼした3つの断片を返す:著者がtalkのQ&Aでだけ口にした但し書き、学会スライドに映っただけで書き残されなかったハイパーパラメータ、何年も前に別の言語で書かれた前処理の手順。再現に必要なものは、語られ、映され、別の言語で書かれていた。Penumbraはその3つすべてを持ち帰った。" width="780">
  </picture>
</div>
