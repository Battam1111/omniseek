...
**しかも複利で効く。** エージェントが持ち帰ったものはすべて永続的な地図に置かれる:次の質問は前の質問の終点から始まり、既に見たミラーはひとつの声に畳まれ、セッションが終わっても常設の監視が見張り続け、エージェントが下した同一性の裁定はこの地図を恒久的に賢くする。検索は一回のクエリ、Penumbraは記憶である。
