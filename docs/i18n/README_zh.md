...
表层之下:一期播客的第 47 分钟有你需要的洞察,从没被转写过。一锤定音的那个数字,在一张幻灯片上停了三秒,被展示却从没被说出口。你苦寻已久的答案,几年前就有人写了出来,用的是一门你不懂的语言。
...
**为什么碰不到?** 锁在音频、视频、图片里,文本搜索解析不了。用你不懂的语言写成。藏在登录墙后面。而且是流动的:昨天还在的帖子,明天可能就没了。无论你用什么工具,撞上的都是同样的壁垒。

**Penumbra 穿过它们。**

一个自托管的深度检索引擎。转写音频,消化文档,从视频和图像中提取,跨语言阅读,遍历引用图谱,监控变化。一个不断生长的开源策展源目录(目前数百个,任何人可扩展),横跨深层网络。讲 MCP 协议,任何 AI agent、工作流或应用直接接入。
...
<div align="center">
  <picture>
    ...
    <img src="../../assets/demo-zh-light.png" alt="一个实例:你的 agent 问一个东西是不是真有他们说的那么好。普通搜索(官网、评测、文档)都说好。Penumbra 伸进表层之下,捞回表层漏掉的三块:一个真用过的工程师在播客里随口说的"一上规模就崩"、演示视频里一闪而过又从没说出口的报错、撞过同一堵墙的人用另一种语言写下的绕法。它们没说的那部分,被说过、被展示过、用另一种语言写过;Penumbra 把三块都带了回来。" width="780">
  </picture>
</div>
