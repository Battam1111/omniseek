...
表层之下:一期播客的第 47 分钟有你需要的洞察,从没被转写过。让它复现的那个关键设置,在一张幻灯片上一闪三秒,而整套幻灯片从没公开过。你正要称之为"新"的方法,几年前就有人用一门你不懂的语言解了出来。
...
<div align="center">
  <picture>
    ...
    <img src="../../assets/demo-light.png" alt="一个实例:你的 agent 问一个结果为什么复现不了。普通搜索(论文、README、博客)都说它能用。Penumbra 伸进表层之下,捞回表层漏掉的三块:作者只在 talk 的 Q&A 里说出口的一个限制、在会议幻灯片上闪过却从没写下的超参、几年前用另一种语言写好的预处理步骤。让它复现的东西被说过、被展示过、用另一种语言写过;Penumbra 把三块都带了回来。" width="780">
  </picture>
</div>
