...
`penumbra_list_sources(domain=..., query=...)` narrows to the sources that actually match, each
with a description.
...
| Call | When |
|------|------|
| `penumbra_search_ranked` | The default. Deduped, ranked, cross-lingual recall folded into one list. |
| `penumbra_search` | You want each source's raw bucket separately, to compare source-by-source. |
| `penumbra_fetch` | Drill ONE named source, unbounded. The escape hatch for a slow or walled source a broad sweep would drop. |

## Walled sources: name them, don't sweep them

A broad `penumbra_search_ranked` / `penumbra_search` call is deadline-bounded and skips
...
`penumbra_search_ranked` results carry `metadata.also_in`: other sources mirroring the same
