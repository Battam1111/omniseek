...
> **Status: pre-release.** This document states the privacy posture of OmniSeek as it prepares for
...
> deployment satisfies any specific privacy regime. An operator who self-hosts OmniSeek is responsible
...
OmniSeek sends **nothing home**. It transmits no usage data, no metrics, and no crash reports to the
...
OmniSeek runs on the **operator's own machine** (the server side). The operator controls all of it:
the host, the process, the cache, the logs, and the data. There is no hosted service in the loop and
no account with this project. Whatever OmniSeek stores, it stores on the operator's host, under the
operator's control.
...
- API keys and credentials live under `~/.omniseek/credentials` on the operator's machine.
...
OmniSeek reaches in two ways, and the results go to one place:
...
OmniSeek's wall-crossing (UNWALL) is bounded to **credentialed access the operator is themselves
...
OmniSeek is released under **Apache-2.0**. What ships is the **general engine plus general adapters**.
...
OmniSeek accesses third-party sites. An operator is responsible for complying, in their own
