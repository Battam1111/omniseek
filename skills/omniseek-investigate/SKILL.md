...
WAVE 1: `omniseek_search(query="<product> review")` + `omniseek_search(query="<product> alternative")`.
WAVE 2: walled community sources + `omniseek_read` for official + critical pages.
...
If a xiaohongshu note URL appears, `omniseek_read(url)` retrieves full note + comment thread.
...
- Max 3 `omniseek_search` per gather (each fans out to many sources)
...
- **Count INDEPENDENT corroboration**: corroboration counts source NAMES, not backends.
  Cross-reference also_in to dedup mirrors. `omniseek_graph(view="voices", args={"doc_ids": [...]})`
  ...
  `omniseek_ruling(action=create)` so the `working` policy collapses it thereafter. Relations
  judged FROM content (a filing says A acquired B) persist via `omniseek_statement` (directed,
  typed, attributed with note + source doc; identity types stay with `omniseek_ruling`).
...
- **Reference-class base rate**: for "join this lab / take this path", estimate the denominator
  with `omniseek_institution_cohort` before trusting anecdote.
