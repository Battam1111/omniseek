...
TIMEOUT = 20
CRED = Path.home() / ".omniseek" / "credentials" / "github.json"
...
def _load_token() -> Optional[str]:
    """The GitHub token from ~/.omniseek/credentials/github.json (``{"token": "..."}``).
...
# ~/.omniseek/credentials/github.json is dropped on the host.
_token = _load_token()
