...
working WITHOUT opening any dangerous range. Extra allowed CIDRs come from OMNISEEK_ALLOW_NETS
...
def _allow_nets() -> tuple:
    ...
    for tok in os.environ.get("OMNISEEK_ALLOW_NETS", "").replace(",", " ").split():
        try:
...
# still opt in via OMNISEEK_ALLOW_NETS.
...
# the full OS resolver duration (~10-30s). Bound it (OMNISEEK_DNS_TIMEOUT_S, default 5s).
try:
    _DNS_TIMEOUT_S = float(os.environ.get("OMNISEEK_DNS_TIMEOUT_S", "5") or 5)
except ValueError:
    _DNS_TIMEOUT_S = 5.0
...
def resolve_pin(url: str) -> "tuple[Optional[str], Optional[str], Optional[str]]":
    ...
    # A PROXY FAKE-IP (198.18/15 or OMNISEEK_ALLOW_NETS) must NOT be pinned: the connection goes to the
