...
from omniseek.core.sources.api._bulk_funding import BulkFundingBase
...
class OecdAiPolicyAdapter(BulkFundingBase):
    ...
    description = (
        ...
        "这是对 谁在进入导航册 的 MONITOR 视角, 非全史注册表. 命名钻取 (omniseek_search 单源 raw)."
    )
    explicit_only = (
        "OECD.AI 政策倡议 最新切片 (最新 ~300 项, 冷启动抓取较慢约 45s; 无全文检索故取最新而非全量); "
        "命名钻取 (omniseek_search 单源 raw); 月级刷新"
    )
    domains = ["policy"]
    regions = ["global"]
