...
class CIHRGrantsAdapter(BulkFundingBase):
    ...
    description = (
        ...
        "机构/系 + 金额(CAD) + program + 主题/类别 + 标题/摘要/关键词. 仅收 AI/ML/NLP 相关切片. 命名钻取 (omniseek_search 单源 raw)."
    )
    explicit_only = "CIHR 加拿大健康经费 临床-AI/NLP 切片 (bulk XLSX, 命名钻取 (omniseek_search 单源 raw)); 月级刷新"
    domains = ["funding"]
    regions = ["ca"]
    modes = ["STRUCTURE"]
    _version = "FY202526"
