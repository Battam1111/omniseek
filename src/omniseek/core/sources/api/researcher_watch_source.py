...
`~/.omniseek/credentials/researcher_watch.json`:
    [
...
from omniseek.core.normalize import Document, keyword_score_filter, mk_signal
...
def _load_watch_list() -> list[tuple[str, str, str]]:
    ...
    cfg = Path(platformdirs.user_config_dir("omniseek")) / "credentials" / "researcher_watch.json"
    # Also accept the credentials dir layout used elsewhere (~/.omniseek/credentials)
    alt = Path.home() / ".omniseek" / "credentials" / "researcher_watch.json"
    for path in (alt, cfg):
        if path.exists():
            try:
...
class ResearcherWatchAdapter:
    ...
    async def asearch(self, query: str, limit: int = 10) -> list[Document]:
        ...
        async), so the live async omniseek_search path awaits it DIRECTLY instead of parking a pool thread
