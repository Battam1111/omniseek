...
DOE, DARPA, IARPA, NIST, NIH, ...), not just one. OmniSeek's "find funding to apply for"
...
the Mac eye host, which reaches it. Live-verify the oppHits field names from OmniSeek host
(omniseek_search 单源钻取) before trusting them. Recon trail: brain note eye-free-api-probe-round2-2026-06-21.
"""
...
class GrantsGovAdapter(BaseScrapeAdapter):
    ...
    async def _araw_fetch(self, query: str, limit: int) -> Optional[Any]:
        # Async twin of _raw_fetch: byte-faithful mirror; only http.post_json -> await http.apost_json.
        body = {
            "keyword": query or "",
            "rows": max(1, min(int(limit), _MAX_ROWS)),
            "oppStatuses": "posted|forecasted",  # applyable now + upcoming
