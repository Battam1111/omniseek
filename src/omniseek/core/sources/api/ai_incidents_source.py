...
the source-report URLs. OmniSeek's safety-STRUCTURE reinforcement: safety previously rested ONLY
...
are the drill-in handles (omniseek_read them for the underlying news source). backend="aiid".
    ...
    def _incident_to_doc(self, inc: Any) -> Optional[Document]:
        ...
        return Document(
            ...
            metadata={
                ...
                # drill-in handles: the underlying source articles (omniseek_read them)
                "reports": reports,
                "raw": jsonsafe(inc),
            },
        )
