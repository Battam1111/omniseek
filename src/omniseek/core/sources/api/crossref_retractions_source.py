...
publisher, retraction date, and the original authors. It complements ``omniseek_paper_enrich`` (which
...
NLP feed. explicit_only: named via omniseek_search 单源钻取 (a topic-filtered integrity probe), not the broad sweep.
"""
...
# auth.contact_email() (~/.omniseek/credentials/contact.json or OMNISEEK_CONTACT_EMAIL), never in-tree.
