...
WHAT IS PUBLIC vs WALLED (verified live 2026-07-11, event EMNLP 2023):
  ...
  * The video is a PUBLIC HLS master playlist on assets.underline.io (unsigned,
    ...
    ``omniseek_transcribe`` ffmpeg-decodes an https .m3u8 directly (its protocol whitelist
...
class UnderlineTalksAdapter(BaseScrapeAdapter):
    ...
    async def _aresolve(lecture_id: str) -> Optional[dict]:
        ...
        from omniseek.core import http  # lazy: mirror _resolve's off-boot discipline (this adapter self-registers at import)
    ...
    def _payload_to_document(payload: dict) -> Optional[Document]:
        ...
        # TRANSCRIBE handle: omniseek_transcribe ffmpeg-decodes an https .m3u8 directly.
        ...
        return Document(
            ...
            metadata={
                ...
                "has_video": has_video,
                # the durable TRANSCRIBE handle (public .m3u8; pass to omniseek_transcribe):
                "transcribe_url": playlist if has_video else None,
                # paper/slides/transcript downloads are login-walled; the paper is on the
                # ACL Anthology (omniseek_anthology). We surface the DOI, not a walled url.
...
def _build_content(*, abstract: str, event_name: Optional[str], poster: bool,
    ...
    if has_video:
        lines.append(
            ...
            "pass this lecture's video URL to omniseek_transcribe to transcribe it with local "
