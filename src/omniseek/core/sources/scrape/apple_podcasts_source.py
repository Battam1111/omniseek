...
pull a real episode .mp3 from that feed and run ``omniseek_transcribe`` on it to get
...
class ApplePodcastsAdapter(BaseScrapeAdapter):
    ...
    description = "Apple Podcasts: find a podcast show + its RSS feedUrl (iTunes Search, keyless); pull an episode .mp3 for omniseek_transcribe"
    cache_ttl = 900
    kind = "lookup"
    domains = ["podcast"]
    ...
    def _to_documents(self, raw: Any, query: str, limit: int) -> list[Document]:
        ...
        for idx, item in enumerate(results[:limit]):
            ...
            # feed the agent pulls an episode .mp3 from to run omniseek_transcribe.
            content_lines = []
            if genres:
            ...
            if feed_url:
                content_lines.append(f"RSS feed (pull an episode .mp3 for omniseek_transcribe): {feed_url}")
            else:
            ...
            # enclosure .mp3 so the agent can omniseek_transcribe it without a second hop.
            if idx == 0 and feed_url:
                mp3 = _latest_episode_mp3(feed_url)
                if mp3:
                    metadata["latest_episode_mp3"] = mp3
                    content += f"\nLatest episode audio (ready for omniseek_transcribe): {mp3}"
