...
SIRS (0-200), Alberta its own. explicit_only: named via omniseek_search 单源钻取 after routing to the immigration
...
class OinpInvitationsAdapter(_CaPnpBase):
    ...
    explicit_only = "OINP 安省提名抽签历史 (HTML 抓取, 命名钻取 (omniseek_search 单源 raw)); 省提名非联邦 EE"
...
class BcpnpInvitationsAdapter(_CaPnpBase):
    ...
    explicit_only = "BCPNP BC 省提名抽签 + SIRS 分布 (HTML 抓取, 命名钻取 (omniseek_search 单源 raw)); SIRS 非联邦 CRS"
...
class AaipDrawsAdapter(_CaPnpBase):
    ...
    explicit_only = "AAIP 阿省提名抽签历史 (HTML 抓取, 命名钻取 (omniseek_search 单源 raw)); 省提名非联邦 EE"
...
class MpnpDrawsAdapter(_CaPnpBase):
    ...
    explicit_only = "MPNP 曼省提名 EOI 抽签历史 (HTML 抓取, 命名钻取 (omniseek_search 单源 raw)); 省提名非联邦 EE"
