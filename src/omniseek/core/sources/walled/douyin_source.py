...
Mechanism (verified 2026-06-22, decoded from MediaCrawler media_platform/douyin/client.py):
  ...
  * data[] items carry aweme_info (or aweme_mix_info.mix_items[0]); each = one video doc. The
    ...
    discovery signal; the agent can then omniseek_transcribe a promising video's url for the speech.
...
class DouyinAdapter(BaseCDPAdapter):
    ...
    explicit_only = "抖音 walled (isolated 9225 小号 session, account-rate-sensitive); named via omniseek_search 单源钻取"
    cdp_url = DOUYIN_CDP
    cdp_timeout = 60
    ...
    def _doc_from_item(item: dict) -> Optional[Document]:
        ...
        # so the durable handle stays the canonical /video/<id> page url (also omniseek_transcribe's target).
        media: list[str] = []
        ...
        return Document(
            ...
            title=title,
            content=desc or "(无文案; 正文在视频里 — 可对此 url 调 omniseek_transcribe 转写语音)",
            author=nickname,
            date=date,
