...
thread (omniseek.serve_http) and, for a manual one-shot, by ``python -m omniseek.core.prewarm``.
...
WHY in-service (2026-06-14): the standalone launchd cron (com.omniseek.core-prewarm) was observed
...
# interval wait immediately (see omniseek.core.lifecycle). Additive: the loop behaves identically until
...
def _main() -> int:
    """Manual one-shot warm (``python -m omniseek.core.prewarm``) — the manual warm CLI, folded in
