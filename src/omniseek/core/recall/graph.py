...
RULINGS_PATH = Path.home() / ".omniseek" / "state" / "graph_rulings.json"
...
# REFUSED — same_as / not_same_as — so identity keeps exactly ONE judgment source (omniseek_ruling).
STATEMENTS_PATH = Path.home() / ".omniseek" / "state" / "graph_statements.json"
...
def _slug_statement_type(type: str) -> str:
    ...
    if slug in _STATEMENT_REFUSED_TYPES:
        raise ValueError(f"type {slug!r} is an IDENTITY judgment; record it via omniseek_ruling "
...
#       similar PROPOSES (top-k by RANK) -> the agent verifies -> omniseek_ruling records -> the working
...
def similar(anchor: str, k: int = 10) -> dict:
    ...
    DELIBERATE NON-GOAL (P7): vec_thin does NOT feed omniseek_search's recall arm — that fold is a separate
    ...
    return {
        ...
        "note": ("candidates are PROPOSALS from embedding proximity, never collapsed by any policy; "
                 "verify, then record a ruling via omniseek_ruling"),
        "coverage": coverage,
        "capped": capped,
    }
...
def describe_views() -> dict:
    ...
    _OMNISEEK_VERBS uses). This is what a no-view dispatch returns, so the surface self-describes."""
    out: dict = {}
...
def _coerce_arg(value, param: "inspect.Parameter"):
    ...
    from omniseek.core.normalize import _coerce_bool, _coerce_int
...
def dispatch_view(view: str, args: Optional[dict] = None) -> dict:
    """The ONE mechanical gate behind omniseek_graph's frozen ``(view, args)`` ABI.
