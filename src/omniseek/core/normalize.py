...
class Document(BaseModel):
    ...
    def to_tool_dict(self, *, full: bool = False, content_cap: int = 2000, debug: bool = False) -> dict:
        """Agent-facing serialization for the MCP omniseek_* tools — the single lean projection.
        ...
        ``full=False`` (DISCOVERY — omniseek_search / omniseek_search_ranked, where the agent triages
        ...
        (``omniseek_add_url`` on this doc's ``url``) for the whole text. ``full=True`` (DRILL-DOWN —
        omniseek_fetch / omniseek_add_url, a source/URL the agent already chose) keeps content whole.
        """
...
def keyword_score_filter(docs: list[Document], query: str) -> list[Document]:
    ...
    Delegates to ``omniseek.core.relevance`` (BM25-shaped: tf saturation + length
    ...
    """
    from omniseek.core.relevance import filter_rank  # local import: keep normalize dependency-free
