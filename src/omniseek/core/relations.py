...
# resolve_identity's likely_same_person groups (the agent ratifies via omniseek_ruling); institution
...
def _likely_same_person(candidates: list[dict]) -> list[dict]:
    ...
    for (_, src), members in groups.items():
        ...
        out.append({
            ...
            "note": "same name, same backend → consider +-merging into one omniseek_coauthors input",
        })
    return out
...
def _resolve_mints(out: dict) -> tuple[list[dict], list[dict]]:
    ...
    ratifies via omniseek_ruling). Institution STRINGS on candidates mint nothing (no id). Pure."""
    nodes: list[dict] = []
    edges: list[dict] = []
...
def _oa_author_works(author_id: str, fresh: bool = False) -> list[dict]:
    ...
    # repeated omniseek_coauthors over the same people reads disk instead of re-spending the shared key. The
...
def coauthors(authors: list[str], source: str = "openalex", hints: Optional[list[str]] = None,
    ...
    # id straight from the neighborhood, then omniseek_coauthors([id])) without re-resolving a
...
def institution_cohort(institution: str, concept: str = "", year_from: Optional[int] = None,
    ...
    roster is a starting point the agent drills (omniseek_coauthors / homepages), not a verified
