...
never import penumbra.*); this module is the IN-PROCESS half, used only by code already running
...
_WECOM_CREDS_PATH = Path.home() / ".penumbra" / "credentials" / "wecom.json"
_ALERT_DELIVERY_PATH = Path.home() / ".penumbra" / "state" / "alert-delivery.json"


def wecom_push(title: str, body: str) -> bool:
    """POST one 企业微信 (WeCom) group-robot MARKDOWN message via the webhook in
    ~/.penumbra/credentials/wecom.json ({webhook_url}). Captain's channel (desktop + phone).
