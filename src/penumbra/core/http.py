    ...
    if _client is None:
        with _client_lock:
            if _client is None:
                ...
                from penumbra.core import safeurl  # noqa: PLC0415 (lazy: breaks the import cycle)
...
def _request_capped(method: str, url: str, *, timeout: int, headers: dict,
    ...
    # penumbra_add_url -> web_fallback -> http.get attacker path; a 'dns' miss is NOT blocked (the fetch
...
def get_impersonated(url: str, *, timeout: int = DEFAULT_TIMEOUT,
    ...
    # closed the arbitrary-user-URL lane instead (penumbra_add_url -> web_fallback -> safeurl.safe_fetch,
