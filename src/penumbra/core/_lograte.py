...
a FRESH install with no PENUMBRA_CONTACT_EMAIL, where OpenAlex refuses the polite pool, rate-limits
...
_INSTALLED_MARK = "_penumbra_ratelimit"
...
def install_on_root(burst: int = 3, window_s: float = 60.0) -> None:
    ...
    ``penumbra.core.*`` / ``penumbra.core.*`` tree — a filter on a logger would miss propagated
