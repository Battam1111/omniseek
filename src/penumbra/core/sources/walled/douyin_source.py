...
ONCE via VNC into an ISOLATED Chrome profile (port 9225, ~/.penumbra/chrome-douyin) and Penumbra
...
Isolation rationale: identical to xiaohongshu (see docs/walled-sources.md) — a disposable
...
class DouyinAdapter(BaseCDPAdapter):
    ...
    def _to_documents(self, raw: Any, query: str, limit: int) -> list[Document]:
        ...
        if raw.get("status_code") == 2483 or "请先登录" in (raw.get("status_msg") or ""):
            diag.note("douyin.needs_login", body=(
                ...
                "VNC into the host's 抖音 Chrome (port 9225) and re-scan-login a disposable 小号."))
            return []
    ...
    def health_check(self) -> tuple[bool, str]:
        ...
        if not ok:
            return False, f"CDP 9225 down: {msg} (抖音 Chrome)"
        try:
