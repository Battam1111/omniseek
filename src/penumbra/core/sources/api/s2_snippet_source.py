...
targeted lit-review move, not something every broad penumbra_search should pay S2 for. English CS/AI corpus
...
merge). The url is the S2 paper page so ``penumbra_paper_enrich`` / ``penumbra_read`` can drill the full paper.
"""
...
class S2SnippetAdapter:
    ...
    description = (
        ...
        "(论文级) 与 penumbra_paper_enrich (单篇全文) 的中间层: passage 级. 英文 CS/AI 语料. 命名钻取 (penumbra_search 单源 raw)."
    )
    explicit_only = (
        "S2 passage 级全文检索; 命名钻取 (penumbra_search 单源 raw); 段落检索是定向文献综述动作, 不进广扇出 "
    ...
    def fetch_url(self, url: str) -> Optional[Document]:
        # Search-only (passage retrieval): an S2 paper URL is semantic_scholar / penumbra_paper_enrich's
        # job, not a snippet source's. Claim nothing so penumbra_read routes it to the right adapter.
        return None
