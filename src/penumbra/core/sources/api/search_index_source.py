...
from penumbra.core.sources.api._search_backend import asearch_web, backend_ping, search_web
...
def _url_cache_key(source: str, url: str) -> str:
    """Stable key for replaying the search-index snapshot through ``penumbra_read``."""
...
def _store_url_snapshots(source: str, docs: list[Document], ttl: int = 1800) -> None:
    ...
    subsequent ``penumbra_read`` return that reachable evidence without accidentally driving the
