...
beneficial owners). Penumbra's UK filings-STRUCTURE source: resolve a company by name, then
...
The key lives ONLY on the host at ~/.penumbra/credentials/uk_companies_house.json ->
...
class UkCompaniesHouseAdapter(BaseAPIAdapter):
    ...
    def health_check(self) -> tuple[bool, str]:
        ...
        if not self._api_key():
            return False, "no api_key (set ~/.penumbra/credentials/uk_companies_house.json)"
        try:
