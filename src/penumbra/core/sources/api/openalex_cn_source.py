...
Penumbra's paper sources (arxiv / semantic_scholar / crossref / dblp / core / openalex) are
...
the agent never reaches them (a grep of Penumbra showed zero language:zh routing). This is a thin
ROUTING facet over the EXISTING OpenAlex wrap (no new API, keyless, same circuit breaker): it
pins ``language:zh`` and re-labels the docs, giving Penumbra structured Chinese 题录 + abstracts +
OA links that Google can't assemble.

It is the NO-LOGIN STRUCTURE answer to the Chinese-scholarship gap (verified by Penumbra-nologin
...
class OpenAlexCNAdapter(OpenAlexAdapter):
    ...
    description = (
        ...
        "(add inline `type:dissertation`) that Penumbra's English paper sources (arxiv/s2/crossref) "
