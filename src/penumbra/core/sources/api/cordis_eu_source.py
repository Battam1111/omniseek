...
from penumbra.core.sources.api._bulk_funding import UA, BulkFundingBase, is_ai_relevant
...
class CordisEuAdapter(BulkFundingBase):
    ...
    description = (
        ...
        "切片 (~3.1k, telos 视角, 非全 Horizon). 仅活跃的 Horizon Europe (已闭的 H2020 可另建). 命名钻取 (penumbra_search 单源 raw)."
    )
    explicit_only = "CORDIS 欧盟 Horizon Europe 经费 AI/ML/NLP 切片 (bulk CSV zip, 命名钻取 (penumbra_search 单源 raw)); 月级刷新"
    domains = ["funding"]
    regions = ["eu"]
    modes = ["STRUCTURE"]
