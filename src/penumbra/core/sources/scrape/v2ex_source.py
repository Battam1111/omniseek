...
# A plain desktop Chrome UA: the API answers keylessly with this (the shared PenumbraEye UA is
...
class V2exAdapter(BaseScrapeAdapter):
    ...
    async def _aget_topics(slug: str) -> Optional[list]:
        ...
        shared PenumbraEye UA via the headers kwarg (V2EX gates obviously-empty UAs)."""
        url = f"{SHOW_URL}?node_name={quote(slug)}"
