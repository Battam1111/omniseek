...
via ``penumbra_add_url`` and it downloads the PDF (size-capped via penumbra.core.http) and extracts the text
...
class PdfAdapter:
    ...
    description = (
        ...
        "so you can read the WHOLE paper, not just the abstract (pair with penumbra_paper_enrich's pdf_url)"
    )
