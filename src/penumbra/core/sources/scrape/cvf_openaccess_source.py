...
returns those listing fields; ``fetch_url`` (or penumbra_read on a paper URL) enriches
...
class CVFOpenAccessAdapter:
    ...
    description = (
        ...
        "列表页给标题/作者/PDF/BibTeX, 摘要在每篇论文页 (对论文 URL 用 penumbra_read 补全)"
    )
    ...
    def _to_doc(p: dict) -> Document:
        ...
        body = abstract or "(no abstract in listing; use penumbra_read on the paper URL)"
        content = "\n".join(parts) + "\n\n" + body
        return Document(
