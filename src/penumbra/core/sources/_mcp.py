...
Auth: optional per-source credentials at ``~/.penumbra/credentials/mcp_<name>.json``
...
from penumbra.core import _netguard, auth, cache, http
...
_CLIENT_INFO = {"name": "penumbra", "version": "0.1"}
...
def load_mcp_headers(name: str) -> "tuple[Optional[dict], bool]":
    """Read per-source MCP credentials at ~/.penumbra/credentials/mcp_<name>.json via the auth.py
