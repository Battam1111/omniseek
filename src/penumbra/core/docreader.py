...
(a 28-slide, 66-image figure-style reference deck): Penumbra digests uploads the
...
image half is delivered to the agent's own vision two ways:
  ...
  - view_images() returns the actual image bytes for IN-BAND delivery (the server
    ...
    full res. Penumbra renders pixels; INTERPRETING them is the agent's vision —
    never a server-side captioner (a weaker model pre-chewing for the real one is
    the fabrication trap). Penumbra = render + structure; agent = see + judge.

Transport: Penumbra runs on the host; the user's files live on client machines.
Convention: ``scp "<file>" <your-host>:penumbra-inbox/`` (any window can),
...
# vision IN-BAND. Tier 4 of the document modality: Penumbra renders pixels, the
...
def view_image_urls(urls, max_images: int = 8, max_dim: int = _VIEW_MAX_DIM) -> dict:
    ...
    document file): for SEEING the post images Penumbra surfaces in a doc's `media` field
