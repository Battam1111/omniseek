...
def load_contract_artifacts(contract_dir: Optional[Path] = None) -> ContractArtifacts:
    ...
    if policy.get("schema") != "penumbra.scheduler-heartbeat-policy/v1":
...
class SchedulerHeartbeat:
    def __init__(
        ...
        *,
        ...
        if isinstance(penumbra_pid, bool) or not isinstance(penumbra_pid, int) or penumbra_pid <= 0:
            raise ValueError("penumbra_pid must be a positive integer")
        ...
        self.penumbra_pid = penumbra_pid
    ...
    def _record(self, *, phase: str) -> dict:
        return {
            "schema": "penumbra.core-scheduler-heartbeat/v1",
            "component_id": "eye-scheduler",
            "producer": {"repo": "penumbra", "build_id": self.build_id},
            "contract": {
            ...
            "penumbra_pid": self.penumbra_pid,
