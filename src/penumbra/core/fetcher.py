...
# The stacked case is the one that decides it: penumbra_gather really does run several broad searches at
...
_SEARCH_WORKERS = 64
# PROCESS-GLOBAL egress bound (loop-agnostic). penumbra_gather runs each batched call on its own thread
# and drives an async tool body (penumbra_search) to completion on a FRESH event loop per call
# (server.penumbra_gather). anyio's default thread limiter is PER-LOOP, so N stacked gather calls get N
...
# penumbra_gather child). AnyIO's worker limiter is loop-local, so acquiring _EGRESS_SEM *inside* a
...
# starved even penumbra_sources. Poll the non-blocking, process-global semaphore while still a coroutine,
    ...
    def _work() -> tuple[list[Document], list]:
        ...
        try:
            ...
            if not cache_only:  # Path A: index enumerable docs (skip on a cache-only collect, mirroring
                from penumbra.core import recall  # search_many._one; a cache_only read has NO side effects)
...
def param_hint(name: str) -> str:
    ...
    (server.penumbra_search's routing_hint) needs only the source name. '' for an unknown / free-text
    source."""
    with _registry_lock:
...
def build_search_plan(catalog: dict, policy: PolicySnapshot, query: str,
    ...
    from penumbra.core import relevance  # local: leaf module (a pure tokenizer over the query arg)
...
# REAL S4c-2 go/no-go flip is the penumbra_search TOOL-BODY change in server.py (penumbra_search is now an
...
# tool-body flip supersedes it. search_many / search_ranked stay SYNC for the other callers (penumbra_gather
...
# =============================================================================================
_ASYNC_FANOUT = False  # PLACEHOLDER, never read; superseded by the penumbra_search async tool-body flip (S4c-2, server.py).
...
async def _dispatch_search(adapter: "SourceAdapter", query: str, limit: int) -> list:
    ...
    # pool limiter is per-loop and penumbra_gather spins a fresh loop per call, so it alone cannot bound
...
def async_fanout_shadow_probe() -> None:
    ...
    try:
        ...
        # S4c-1: ALSO shadow the RANKED path (penumbra_search's DEFAULT), the most-used/most-complex twin.
...
def _compact_judgments(node_ids: list[str], stmt_index: dict) -> "tuple[list[dict], bool]":
    ...
    if not stmt_index:
        return [], False
    from penumbra.core.recall.graph import _id_self_label
    seen: set = set()
    out: list[dict] = []
    capped = False
    for nid in node_ids:
...
def _place_graph_presence(ranked: list[Document]) -> None:
    ...
    reflexive search, closing the write-side loop with no penumbra_graph verb invoked.
    ...
    try:
        from penumbra.core.recall import store
        from penumbra.core.recall import graph as _g
        if store._disabled:
            return
        con = store._read_con()
        if con is None:
    ...
    for d, wids, stmt_ids in doc_rows:
        ...
        if counts:
            ...
            stamp["note"] = ("accreted store-memory from past tool runs, not live bibliometrics; "
                             "refresh the live/deep version via the named drill (penumbra_graph / penumbra_coauthors / ...)")
        if judgments:
...
def _selfwarm_revisited(ranked: list[Document]) -> None:
    ...
    try:
        ...
        from penumbra.core import enrich as _enrich
    except Exception:  # noqa: BLE001
        return
...
async def asearch_ranked(
    ...
    DORMANT (S4c-1): NOT wired into penumbra_search or any live path (that flip is S4c-2). Reached only by
...
def fetch_url_with_reason(url: str) -> "tuple[Optional[Document], Optional[str]]":
    ...
    try:
        from penumbra.core import diag, web_fallback
        diag.enable()
