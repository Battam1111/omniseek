...
_SOURCES_DIR = (Path(__file__).resolve().parents[1] / "sources")  # .../penumbra/eye/sources
...
def register_one(family: str, row: dict):
    ...
    if family == "rss":
        from penumbra.core.sources.scrape.rss_bundles_source import _RSSBundle
        return _RSSBundle(
    ...
    if family == "news_scraper":
        from penumbra.core.sources.scrape.news_scraper_source import _ScrapeSite
        return _ScrapeSite(
    ...
    if family == "search_index":
        from penumbra.core.sources.api.search_index_source import _SearchVenue
        return _SearchVenue(
...
def apply_overlay_row(cand: dict) -> dict:
    ...
    from penumbra.core.curator import candidates as _candidates
