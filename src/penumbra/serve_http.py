...
from penumbra.core import lifecycle  # noqa: E402
...
def _raise_fd_limit(want: int = 16384) -> None:
    ...
    penumbra_gather blows through with "Too many open files". The process-global egress semaphore in
...
def main() -> None:
    ...
    _raise_fd_limit()  # a 95-source fan-out under nested penumbra_gather needs headroom over macOS's 256 default
    ...
    # async PORTAL (also S3). No double-writer today: the stdio path (penumbra.server.main -> mcp.run)
